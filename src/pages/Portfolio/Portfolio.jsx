...
const EXTRA_PROJECTS = [
  {
    id: "https://resume-builder-fawn-eight.vercel.app/",
...
const mergeProjects = (fetched) => {
  const existingIds = new Set(fetched.map((p) => p.id));
  const newEntries = EXTRA_PROJECTS.filter((p) => !existingIds.has(p.id));
...
const Portfolio = () => {
  ...
  useEffect(() => {
    fetch("projects.json")
      .then((res) => res.json())
      .then((data) => {
        const merged = mergeProjects(data);
        setProjects(merged);
        setFilteredProjects(
          sortProjectsByCategoryPriority(merged, DEFAULT_FIRST_CATEGORY)
        );
      });
  }, []);
